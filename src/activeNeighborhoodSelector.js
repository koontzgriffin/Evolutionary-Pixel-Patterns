...
function addNeighborhood(divId) {
    const index = vaildActiveNeighborhoods.indexOf(intToBinaryString(parseInt(divId)));

    if (index === -1) {
        // Div is not in the list, add it
        vaildActiveNeighborhoods.push(intToBinaryString(parseInt(divId)));
        console.log(`validNeighborhoods: ${vaildActiveNeighborhoods}`);
        changeColor(divId, 'blue')
    } else {
        // Div is already in the list, remove it
        vaildActiveNeighborhoods.splice(index, 1);
        console.log(`validNeighborhoods after removal: ${vaildActiveNeighborhoods}`);
...
function createCanvas(seed) {
    ...
    const ctx = canvas.getContext('2d');
    if(vaildActiveNeighborhoods.includes(intToBinaryString(seed))){
    ...
    canvasDiv.addEventListener('click', function () {
        ...
        setAllowedActiveNeighborhoodsCookie(vaildActiveNeighborhoods);
    });
...
function clearHandler(){
    vaildActiveNeighborhoods = [];
    setAllowedActiveNeighborhoodsCookie(vaildActiveNeighborhoods);
    location.reload(true);
}

function resetHandler(){
    vaildActiveNeighborhoods = defaultValidActiveNeighborhoods;
    setAllowedActiveNeighborhoodsCookie(vaildActiveNeighborhoods);
    location.reload(true);
}
